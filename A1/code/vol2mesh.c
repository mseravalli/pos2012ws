...
/**
 * @brief This function is used to convert the volume-based topology 
 *      (of Fire input files) to unstructured mesh topology and geometry 
 *      (for vtk format output).
 *
 * The volume topology is received via the input array LCC (link cell-cell). 
 *      One LCC[nghbId][cellId] entry in the array stores the neighboring cell 
 *      of cellId in direction nghbId.
 *      Each volume cell is considered to be a 1x1x1 cube. 
 *      The transformation result is stored in three variables:
 *          elems: 2D array storing the mesh elements. 
 *      An element is defined by 8 points(nodes):
 *             elems[i][NC] -> i-th node of mesh element NC 
 *                  (see also scheme below)
 *      points: 2D array storing the (x,y,z) coordinates for each point (node):
 *              points[0][i] -> x coord of point i; 
 *              points[1][i]->y coord of point i; ...
 *      nodeCnt: the total number of points in the mesh 
 *          (size of second dimension of points array)
 *
...
int vol2mesh( int startIdx, 
     ...
     *              H - 5
     * */
    int node2nghbMap[8][6] = { { 0, 1, 3, 3, 4, 4 }, // node 7 on S nghb, 
                               { 3, 2, 4, 5, 2, 0 }, // node 4 on W nghb, 
                               { 4, 6, 1, 1, 2, 3 }, // node 5 on L nghb
                               { 0, 2, 4, 7, 1, 0 }, 
                               { 3, 7, 0, 5, 5, 0 }, 
                               { 3, 6, 5, 1, 2, 4 }, 
                               { 5, 2, 2, 7, 1, 5 }, 
     ...
     * search a nghb with smaller cell index and derive the coordinates from 
     *  one of its points:
     *      search among the nghbs which do not share this node (otherwise, 
     *          it would have already be defined):
     *              E: y0 = y0E - 1
     *              H: z0 = z0H - 1
     *              N: x0 = x0N + 1
     * */
    ...
    for ( i = 0; i < 3; i++ ) { 
        ...
        if ( ( ( *points )[i] 
                    = ( int * ) malloc( ( elemCnt + 1 ) * sizeof( int ) * 8 ) )
                == NULL ) {
    ...
    for ( i = 0; i < 8; i++ ) {
        if ( ( ( *elems )[i] 
            = ( int * ) malloc( ( elemCnt + 1 ) * sizeof( int ) ) ) == NULL ) {
    ...
    int NC;
    for ( NC = startIdx; NC <= endIdx; NC++ ) { // for each volume cell
        for ( nodeIdx = 0; nodeIdx < 8; nodeIdx++ ) { // for each node
            foundVal = 0;
            ...
            for ( nghbIdx = 0; nghbIdx < 6; nghbIdx = nghbIdx + 2 ) {
                ...
                if ( currNghb < NC ) {
                    ( *elems )[nodeIdx][NC] =
                        ( *elems )
                        [node2nghbMap[nodeIdx][nghbIdx + 1]][currNghb];
                    foundVal = 1; //aka true
                    break;
                }
            }
            ...
            if ( !foundVal ) {
                ...
                if ( nodes == 1 ) { // the first node ever : )
                    ( *points )[0][nodes] = 
                        ( *points )[1][nodes] = ( *points )[2][nodes] = 0;
                } else if ( nodeIdx == 0 ) { 
                    ...
                    for ( coordIdx = 0; coordIdx < 3; coordIdx++ ) {
                        ( *points )[coordIdx][nodes] = 
                            ( *points )[coordIdx][( *elems )[nodeIdx - 1][NC]] + 
